define("@swap/crosschain-utils", ["require", "exports", "@ijstech/eth-wallet", "@swap/global", "@swap/store", "@scom/oswap-cross-chain-bridge-contract"], function (require, exports, eth_wallet_1, global_1, store_1, oswap_cross_chain_bridge_contract_1) {
    "use strict";
    ...
    const getTargetChainTokenMap = (chainId) => {
        let tokenList = store_1.tokenStore.getTokenList(chainId);
    ...
    const getTargetChainTokenInfoObj = async (chainId) => {
        ...
        const tokenList = store_1.tokenStore.getTokenList(chainId).filter((token) => token.address);
        const erc20TokenList = tokenList.filter(token => token.address);
        const nativeToken = tokenList.find(token => !token.address);
        try {
            const erc20 = new eth_wallet_1.Contracts.ERC20(targetChainWallet);
            ...
            if (result) {
                for (let i = 0; i < erc20TokenList.length; i++) {
                    ...
                    if (token.address) {
                        balances[token.address.toLowerCase()] = new eth_wallet_1.BigNumber(result.results[i]).shiftedBy(-token.decimals).toFixed();
                    }
                }
                if (nativeToken) {
